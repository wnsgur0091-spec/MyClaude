...
개선본 평가는 Gemini 호출을 추가하지 않고 기존 재분석 1회에 원본과 개선본을 함께 전달합니다. Gemini가 추천 대상 외 영역이나 전체 구도가 크게 달라졌다고 판단하면 개선본을 폐기하고 원본으로 복구합니다.

현재 기본 모델 `flux-2-klein-4b`의 Workers AI 입력 스키마는 참조 이미지는 지원하지만 mask 입력은 지원하지 않습니다. 마스크를 지원하는 별도 Stable Diffusion 1.5 Inpainting 모델도 있으나, 현재 분석 결과는 의상의 정확한 분할 마스크가 아닌 한 점 좌표만 제공하므로 임의의 사각형 마스크를 생성해 사용하지 않습니다.
