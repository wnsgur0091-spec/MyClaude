...
이 저장소에는 개발 도구(Flutter SDK)가 없는 환경에서 작성되어 `flutter analyze`/`flutter pub get`을 실행해보지 못했습니다. 위 명령으로 gradle wrapper를 생성하고 `flutter analyze`로 한 번 확인해주세요.

Google Calendar를 쓰려면 Firebase/Google Cloud 콘솔에서 OAuth 클라이언트를 만들고 `android/app/google-services.json`(또는 `google_sign_in` Android 설정)을 추가해야 합니다.
...
2. **TimeTree는 서드파티용 공개 조회 API를 더 이상 제공하지 않습니다.** 대신 TimeTree 앱의 "캘린더 내보내기"에서 얻는 ICS/webcal 공유 링크를 온보딩에서 입력받아 구독하는 방식으로 구현했습니다(`lib/services/calendar/timetree_ics_service.dart`). Google Calendar는 공식 REST API(Calendar API v3)를 정상적으로 사용합니다.
